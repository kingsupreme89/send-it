  ...
  return (
    ...
    <div className="flex flex-col gap-4">
      {headline && (
        ...
        >
          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-white/70">Top Story</p>
          ...
          <ul className="flex flex-col gap-2">
            {visibleGames.map((g) => {
              ...
              return (
                <li
                  key={g.id}
                  className="rounded-2xl border border-[var(--border)] bg-white/[0.02] p-3 transition-colors hover:bg-white/[0.04]"
                >
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-medium text-[var(--text)]">
                      {g.winnerIds.map(nameFor).join(' & ')} def. {g.loserIds.map(nameFor).join(' & ')}
                    </span>
                    <span className="flex items-center gap-2">
                      <span className="font-semibold text-[var(--text)]">{g.winnerScore}-{g.loserScore}</span>
                      {canEdit(g) && (
                        <button
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-[var(--text-muted)]">
                    ...
                    <span>·</span>
                    <span>{NFL_TEAMS_BY_ID[g.winnerTeam]?.abbreviation} vs {NFL_TEAMS_BY_ID[g.loserTeam]?.abbreviation}</span>
                    {g.isBlowout && (
                      <span className="rounded-full bg-rose-500/15 px-2 py-0.5 font-semibold text-rose-300">💥 Blowout</span>
                    )}
                    {g.isComeback && (
                      <span className="rounded-full bg-amber-500/15 px-2 py-0.5 font-semibold text-amber-300">🔥 Comeback</span>
                    )}
                  </div>
